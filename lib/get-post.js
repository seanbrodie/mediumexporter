...
module.exports = async function(mediumURL, params = {}) {
  ...
  let outputPath = path.join(__dirname, '../../../', output, story.id)
  ...
  return Promise.all(promises)
    .then(async results => {
      if (!!images.length) {
        ...
        let outputPath = path.join(__dirname, '../../../', output, story.id, 'images')
        createFolder(outputPath)
      ...
      let outputPath = `${output}/${story.id}/index.md`
      if (output) {
        if (!!images.length) {
          outputPath = path.join(output, story.id) + '/index.md'
        }
