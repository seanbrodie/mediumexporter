...
async function loadMediumPost(mediumURL, options = {}) {
	...
	if (mediumURL.match(/^http/i)) {
		...
		const response = (await axios(mediumURL)).data;
...
function processSection(s, images, options = {}) {
	...
	if (s.backgroundImage) {
		...
		const imgsrc = MEDIUM_IMG_CDN + Math.max(imgwidth * 2, 2000) + '/' + s.backgroundImage.id;
		images.push(imgsrc);
...
async function processParagraph(p, images, options = {}) {
	...
	switch (p.type) {
		...
		case 4: // image & caption
			...
			const imgsrc = MEDIUM_IMG_CDN + Math.max(imgwidth * 2, 2000) + '/' + p.metadata.id;
			images.push(imgsrc);
		...
		case 10:
			...
			break;
		case 11:
			return await getGitHubEmbed('https://medium.com/media/' + p.iframe.mediaResourceId, options);
		case 13:
			markup = '\n### ';
			break;
			...
			case 3: // anchor tag
				if (m.userId) {
					...
					if (user.twitterScreenName) {
						...
						addMarkup(markups_array, `[`, `](https://medium.com/@${user.username})`, m.start, m.end);
					}
				} else {
